...
py::array_t<float> gen_normal_map(const py::array_t<float> &scan_x,
                                  const py::array_t<float> &scan_y,
                                  const py::array_t<float> &scan_z)
{
    ...
    auto *pointY = (float *)buf2.ptr;
    auto *pointZ = (float *)buf3.ptr;

    auto *normal_map = (float *)buf4.ptr;
...
#pragma omp parallel for
    ...
    // printf("start gen_normal_map -> gen_normal \n");

    // printf("buf1.size = %d \n", buf1.size);

    int iCount = buf1.size;

    // printf("normal_map[0] = %f\n", normal_map[0]);
    // printf("normal_map[1] = %f\n", normal_map[1]);
    // printf("normal_map[2] = %f\n", normal_map[2]);
    ...
    // printf("normal_map[3] = %f\n", normal_map[3]);
    // printf("normal_map[4] = %f\n", normal_map[4]);
    // printf("normal_map[5] = %f\n", normal_map[5]);

    // reshape array to match input shape
    // normal_map_buffer.resize({buf1.size, 1});
...
// py::array_t<float> gen_normal_map2(const py::array_t<float> &range,
